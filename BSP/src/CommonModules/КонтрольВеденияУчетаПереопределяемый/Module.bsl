...
Процедура ПриОпределенииПроверок(ГруппыПроверок, Проверки) Экспорт
	// Санчес  начало примера
	
	ГруппаПроверок = ГруппыПроверок.Добавить();
	ГруппаПроверок.Наименование                 = НСтр("ru=' Проверки'");
	ГруппаПроверок.Идентификатор                = "_ПроверкиВеденияУчета";
	ГруппаПроверок.КонтекстПроверокВеденияУчета = "_ПроверкиВеденияУчета";
	
	Проверка = Проверки.Добавить();
	Проверка.ИдентификаторГруппы          = ГруппаПроверок.Идентификатор;
	Проверка.Наименование                 = НСтр("ru='Проверка заполнения статуса Оплачено в заявках'");
	Проверка.Причины                      = НСтр("ru='Не введен статус оплачено в заявке'");
	Проверка.Рекомендация                 = НСтр("ru='Установите статус оплачено'");
	Проверка.Идентификатор                = "Демо.ПроверитьСтатусОплачено";
	Проверка.ОбработчикПроверки           = "_СтандартныеПодсистемы.ПроверитьСтатусОплачено";
	Проверка.ДатаНачалаПроверки           = Дата('20140101000000');
	Проверка.ЛимитПроблем                 = 3;
	Проверка.ЗапрещеноИзменениеВажности   = Истина;
	Проверка.КонтекстПроверокВеденияУчета = "_ПроверкиВеденияУчета";
	Проверка.Комментарий                  = НСтр("ru='  Проверка.'");
...
Процедура ПриОпределенииПараметровКолонкиИндикации(ПараметрыКолонкиИндикации, ПолноеИмя) Экспорт
	...
	//// Санчес  начало примера
	//Если ПолноеИмя = Метаданные.Документы._ЗаявкаНаОплату.ПолноеИмя() Тогда
