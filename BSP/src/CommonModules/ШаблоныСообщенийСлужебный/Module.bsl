...
Функция СформироватьСообщение(ПараметрыОтправки) Экспорт
	...
	Если МенеджерОбъекта <> Неопределено Тогда
		МенеджерОбъекта.ПриФормированииСообщения(Сообщение, ПараметрыОтправки.Предмет, ПараметрыШаблона, ПараметрыОтправки.дополнительныеПараметры.Контакт);
	КонецЕсли;
...
//++Гаранин
Функция СформироватьСообщениеИОтправитьВторойСогласователь(ПараметрыОтправки) Экспорт
...
Процедура ЗаполнитьПолучателейСообщения(ПараметрыОтправки, ПараметрыШаблона, Результат, МенеджерОбъекта)
	...
	Если ПараметрыШаблона.ТипШаблона = "Письмо" Тогда
		...
		Если МенеджерОбъекта <> Неопределено Тогда
			МенеджерОбъекта.ПриЗаполненииПочтыПолучателейВСообщении(Получатели, ПредметСообщения, ПараметрыОтправки.ДополнительныеПараметры.Контакт);
		КонецЕсли;
...
Процедура СформироватьСообщениеВФоне(ПараметрыВызоваСервера, АдресХранилища,Контакт = неопределено) Экспорт
...
//++Гаранин
Процедура СформироватьСообщениеВФонеВторойСогласователь(ПараметрыВызоваСервера, АдресХранилища) Экспорт

	ПараметрыОтправки = ПараметрыВызоваСервера.ПараметрыОтправки;
	ВидСообщения = ПараметрыВызоваСервера.ВидСообщения;
	
	Если ПараметрыОтправки.ДополнительныеПараметры.ОтправитьСразу Тогда
		Результат = СформироватьСообщениеИОтправитьВторойСогласователь(ПараметрыОтправки);
		ПоместитьВоВременноеХранилище(Результат, АдресХранилища);
	Иначе
		Сообщение = ШаблоныСообщений.СформироватьСообщение(ПараметрыОтправки.Шаблон, ПараметрыОтправки.Предмет, ПараметрыОтправки.УникальныйИдентификатор, ПараметрыОтправки.ДополнительныеПараметры);
		Если ВидСообщения = "Письмо" Тогда
